...
func TestInitPreviewShowsWhatWouldBeCreated(t *testing.T) {
	...
	got := initPreview(own, [][]byte{own, other}, []rosterDevice{{pub: dev, label: "node-b"}}, 2, "argus lock init --confirm "+strings.Join(args, " "))
...
// The re-run hint must reproduce this invocation, not fall back to flag defaults:
// dropping --gen-disablements would silently change the disablement-secret count.
func TestInitConfirmRerunPreservesFlags(t *testing.T) {
	own := keyfmt.SignerKey.Encode(bytes.Repeat([]byte{0x01}, 32))
	cmd := newLockInitCmd()
	if err := cmd.ParseFlags([]string{"--gen-disablements", "2", "--socket", "/tmp/x.sock", own}); err != nil {
		t.Fatal(err)
	}

	got := initConfirmRerun(cmd, cmd.Flags().Args())

	for _, want := range []string{"--confirm", "--gen-disablements 2", "--socket /tmp/x.sock", own} {
		if !strings.Contains(got, want) {
			t.Errorf("rerun must contain %q, got: %s", want, got)
		}
	}
	if strings.Contains(got, "--confirm --confirm") {
		t.Errorf("--confirm must not be duplicated, got: %s", got)
	}
}
