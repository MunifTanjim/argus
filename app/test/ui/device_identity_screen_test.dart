...
final _fixedPubKey = 'devpub:${_hex(_fixedPub)}';
...
void main() {
  ...
  testWidgets(
    ...
    expect(find.textContaining(_fixedPubKey), findsWidgets);
    expect(
        find.textContaining('argus lock sign $_fixedPubKey'), findsOneWidget);
  });
  ...
  testWidgets('verify section shows signer-set fingerprint words and signer pubkeys',
    ...
    // The signer pubkey (sigpub: + hex, matching `argus lock status`) should appear.
    expect(find.textContaining('sigpub:${_hex(_fixedSigner)}'), findsOneWidget);
  });
