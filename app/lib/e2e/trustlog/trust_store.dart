...
/// Holds a verified chain pinned to an out-of-band genesis head. ingest adopts a
...
/// regression behind the last-seen head detectable.
class TrustStore {
  TrustStore(Uint8List genesisHead)
      : _genesisHead = Uint8List.fromList(genesisHead),
        _tofu = false;

  /// A genesis-unpinned store (Trust-On-First-Use): the first ingest adopts a
  /// fully-verified chain and pins its genesis head; later ingests are pinned.
  TrustStore.tofu()
      : _genesisHead = null,
        _tofu = true;

  Uint8List? _genesisHead; // set on the first TOFU adopt
  final bool _tofu;
  TrustLog? _log;
  List<Entry>? _entries;
  ...
  /// Returns whether the verified HEAD advanced. Identical chain → false (no-op);
  ...
  Future<bool> ingest(Uint8List chainBytes) async {
    ...
    if (_genesisHead == null) {
      // TOFU first adopt: verify the chain internally (Load), then pin its head.
    ...
    if (!_eq(hashEntry(entries.first), _genesisHead!)) {
      throw const FormatException('trustlog: candidate genesis does not match pinned head');
    }
