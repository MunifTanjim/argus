...
type fakeNode struct {
	...
	beforeMsg2 func()
	// plaintext, when true, skips the Noise handshake: the first sealed (=cleartext)
	// request frame lazily creates an identity channel instead.
	plaintext bool
...
type fakeMultiGateway struct {
	peer   *api.Peer
	mu     sync.Mutex           // guards nodes/order/gotHandshake/syncCallCount: addNode races the peer read loop
	...
	// gotHandshake is set when any e2e.handshake frame is received; used to assert
	// that plaintext clients send none.
	gotHandshake  bool
	syncCallCount int // incremented each time trustlog.sync is dispatched; guarded by mu
}
...
func (g *fakeMultiGateway) onFrame(_ *api.Peer, f api.RelayFrame) {
	...
	if n.ch == nil {
		...
		n.ch = api.NewPlainChannel(f.Route.ChanID)
	}
	params, err := n.ch.OpenParams(f)
	if err != nil {
		return
	}
	if f.Method == api.MethodNodeIdentify {
		res, _ := json.Marshal(api.IdentifyResult{ID: n.id, Tip: n.tip})
		rf, _ := n.ch.SealResponseFrame(f.ID, res, nil)
...
func (g *fakeMultiGateway) trustSyncCount() int {
	g.mu.Lock()
	defer g.mu.Unlock()
	return g.syncCallCount
}

func TestTrustChangedNudgeKicksSync(t *testing.T) {
	...
	node := &fakeNode{id: "n1", key: mustKP(t)}
	node.handle = func(string, json.RawMessage) (json.RawMessage, *api.RPCError, *fakeNote) {
		return nil, nil, nil
	}
	...
	// Connect calls syncTrustLog once synchronously; record that baseline.
	baseCount := gw.trustSyncCount()

	// Deliver a NodeEventTrustChanged nudge via the gateway notification channel
	// (the same path onPeerNotify handles). The Node field is irrelevant for this event type.
	if err := gw.peer.Notify(api.MethodNodeEvent, api.NodeEvent{Type: api.NodeEventTrustChanged}); err != nil {
		t.Fatalf("Notify: %v", err)
	}

	// The nudge must trigger a prompt sync well within the 1-hour interval.
	deadline := time.After(3 * time.Second)
	for {
		select {
		case <-time.After(10 * time.Millisecond):
			if gw.trustSyncCount() > baseCount {
				return // sync happened promptly
			}
		case <-deadline:
			t.Fatalf("trust-log sync was not triggered within deadline after NodeEventTrustChanged (baseCount=%d, now=%d)", baseCount, gw.trustSyncCount())
		}
	}
}
...
// debugAuthTip returns a copy of the tip learned over the authenticated channel
// for the given identity public key. Test-only.
func (m *E2EClient) debugAuthTip(identityPub []byte) []byte {
	tip, _ := m.authTipEntry(identityPub)
	return tip
}
...
// TestClientKnownSetCachedByTip verifies that checkTipConsistencyWithChain caches the
// entry-hash set keyed on the resolved chain tip and reuses it on an unchanged tick,
// then rebuilds it when the tip advances.
func TestClientKnownSetCachedByTip(t *testing.T) {
	signer, _ := trustlog.GenerateSigner()
	lg, _ := trustlog.NewGenesis([][]byte{signer.Public}, signer, nil)
	head := lg.Tip()
	...
	// Add a tip directly so the early-return guard does not skip the body.
	key := string(bytes.Repeat([]byte{0xab}, 32))
	m.mu.Lock()
	m.authTip[key] = chainTip
	m.mu.Unlock()

	// Prime: first pass builds and caches the known-set.
	m.checkTipConsistencyWithChain(chain, chainTip)
	m.mu.Lock()
	tip1 := append([]byte(nil), m.knownSetTip...)
	set1 := m.knownSet
	m.mu.Unlock()
	...
	const sentinel = "\xff_tip_cache_sentinel"
	m.mu.Lock()
	m.knownSet[sentinel] = true
	m.mu.Unlock()

	m.checkTipConsistencyWithChain(chain, chainTip)

	m.mu.Lock()
	hasSentinel := m.knownSet[sentinel]
	m.mu.Unlock()
	if !hasSentinel {
	...
	m.checkTipConsistencyWithChain(newChain, newChainTip)

	m.mu.Lock()
	hasSentinelAfter := m.knownSet[sentinel]
	m.mu.Unlock()
	if hasSentinelAfter {
...
func TestAuthTipLearnedOverChannel(t *testing.T) {
	tip1 := bytes.Repeat([]byte{0x01}, 32)
	tip2 := bytes.Repeat([]byte{0x02}, 32)

	n1 := &fakeNode{id: "na", key: mustKP(t), tip: tip1}
	n2 := &fakeNode{id: "nb", key: mustKP(t), tip: tip2}
	...
	if got := c.debugAuthTip(n1.key.Public); !bytes.Equal(got, tip1) {
		t.Errorf("node na: authTip = %x, want %x", got, tip1)
	}
	if got := c.debugAuthTip(n2.key.Public); !bytes.Equal(got, tip2) {
		t.Errorf("node nb: authTip = %x, want %x", got, tip2)
	}
}

// TestTipConsistencyOverChannel drives the authenticated tip cross-check: a node
// answering node.identify with a tip absent from the client's resolved chain trips
// equivocation after the miss threshold, while all-in-chain tips stay quiet.
func TestTipConsistencyOverChannel(t *testing.T) {
	...
	build := func(t *testing.T) (nodeA, nodeB *fakeNode, chain, genesis, chainHead []byte) {
		...
		entries := lg.Entries()
		chain = trustlog.MarshalChain(entries)
		chainHead = trustlog.HashEntry(&entries[len(entries)-1])
		noop := func(string, json.RawMessage) (json.RawMessage, *api.RPCError, *fakeNote) {
			return nil, nil, nil
		}
		nodeA.handle = noop
		nodeB.handle = noop
		return nodeA, nodeB, chain, genesis, chainHead
	}

	connect := func(t *testing.T, nodeA, nodeB *fakeNode, chain, genesis []byte) *E2EClient {
		t.Helper()
	...
	t.Run("off-chain-tip-trips", func(t *testing.T) {
		nodeA, nodeB, chain, genesis, chainHead := build(t)
		nodeA.tip = chainHead
		nodeB.tip = bytes.Repeat([]byte{0x99}, 32) // never in the resolved chain
		c := connect(t, nodeA, nodeB, chain, genesis)
		for i := 0; i < 3; i++ { // > miss threshold
			c.syncTrustLog()
		}
		if !c.Equivocation() {
			t.Fatal("an off-chain tip read over the channel must trip equivocation")
		}
	})

	t.Run("in-chain-tips-quiet", func(t *testing.T) {
		nodeA, nodeB, chain, genesis, chainHead := build(t)
		nodeA.tip = chainHead
		nodeB.tip = chainHead
		c := connect(t, nodeA, nodeB, chain, genesis)
		...
		if c.Equivocation() {
			t.Fatal("all in-chain tips must not trip equivocation")
		}
	})
}

func TestE2EClientPlaintextNoHandshake(t *testing.T) {
	n1 := &fakeNode{
		id:        "n1",
		key:       mustKP(t),
		plaintext: true,
	...
	c, err := NewE2EClientPlain(clientConn)
	if err != nil {
		t.Fatalf("NewE2EClientPlain: %v", err)
	}
	defer c.Close()
	...
	var sessions []any
	if err := c.callNode("n1", api.MethodSessionsList, nil, &sessions); err != nil {
		t.Fatalf("callNode sessions.list: %v", err)
	}

	gw.mu.Lock()
	hadHandshake := gw.gotHandshake
	gw.mu.Unlock()
	if hadHandshake {
		t.Error("plaintext client must not send e2e.handshake frame")
	}
}

// TestPlaintextClientOpensChannelWithNoIdentityKey is the regression guard for
// Risk A: a node running without E2EE never calls SetIdentityKey, so its roster
// entry has IdentityPubKey == "". The plaintext client must not skip such nodes —
// it must open a channel and complete a call through it.
func TestPlaintextClientOpensChannelWithNoIdentityKey(t *testing.T) {
	n1 := &fakeNode{
		id:        "n1",
		key:       e2e.KeyPair{}, // zero Public → nodeDescriptor produces IdentityPubKey: ""
		plaintext: true,
		handle: func(_ string, _ json.RawMessage) (json.RawMessage, *api.RPCError, *fakeNote) {
			return json.RawMessage(`[]`), nil, nil
		},
	...
	snap := c.byNodeSnapshot()
	if _, ok := snap["n1"]; !ok {
		t.Fatal("plaintext client must open a channel to a node with empty IdentityPubKey")
	}

	var sessions []any
	if err := c.callNode("n1", api.MethodSessionsList, nil, &sessions); err != nil {
		t.Fatalf("callNode sessions.list on keyless node: %v", err)
	}
