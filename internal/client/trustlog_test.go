...
// An orphan entry (ancestor absent) must not trigger a retry: exactly one sync,
// returning whatever assembles.
...
// placed; retention keeps Y's entries so assembly of [genesis, devY, D] succeeds.
...
func TestUnplacedWarningSuppressedWhenUnchanged(t *testing.T) {
	...
	orphans, _ := trustlog.ChainEntries(trustlog.MarshalChain(all[1:]))
