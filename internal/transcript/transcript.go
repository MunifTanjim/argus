// Package transcript defines argus's stable, tool-agnostic display model for a
// coding session's conversation: the chunk/item types shipped over RPC and
// rendered by clients. Adapters (claude-code, codex, ...) map their own
// transcript formats into these types, so this boundary stays fixed regardless
// of any one tool's on-disk schema.
package transcript
