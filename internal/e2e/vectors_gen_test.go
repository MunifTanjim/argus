...
func TestGenerateVectors(t *testing.T) {
	...
	enfNodeAPub := fixedKeypair(suite, enfNodeASeed).Public
	...
	ddNon := mustGenSignerSeed(t, 0xD2)
	ddB := mustGenSignerSeed(t, 0xD3)
	ddSecret := bytesFill(0xD4)
	...
	out := map[string]any{
		...
		"fork_choice": map[string]any{
			"cosigned_shorter_beats_longer": map[string]any{
				...
				"cur":                      b64(fcCurChain),
				"cand":                     b64(fcCandChain),
				"winner_tip":               b64(fcWinnerTip),
				"winner_signers":           []string{b64(fcA.Public), b64(fcB.Public)},
				"winner_devices":           []string{},
			...
			"puppet_attack": map[string]any{
				...
				"honest_chain":    b64(ppHonestChain),
				"attacker_chain":  b64(ppAttackerChain),
				"winner_tip":      b64(ppWinnerTip),
				"winner_signer_a": b64(ppA.Public),
				"winner_signer_b": b64(ppB.Public),
				"loser_signer_c":  b64(ppC.Public),
				"puppet_p1":       b64(ppP1.Public),
			...
			"tiebreak": map[string]any{
				...
				"winner_tip":   b64(tbWinnerTip),
			},
			"revoke_with_replacement": map[string]any{
				"genesis_hash":    b64(rwrGenesisHash),
				"honest_chain":    b64(rwrHonestChain),
				"c_branch":        b64(rwrCBranchChain),
				"winner_tip":      b64(rwrWinnerTip),
				"winner_signer_a": b64(rwrA.Public),
				"winner_signer_b": b64(rwrB.Public),
				"winner_signer_d": b64(rwrD.Public),
				"loser_signer_c":  b64(rwrC.Public),
				"loser_dev_c":     b64(rwrDevC),
			},
			"disablement_dominance": map[string]any{
				"genesis_hash":   b64(ddGenesisHash),
				"disabled_chain": b64(ddDisabledChain),
				"attacker_chain": b64(ddAttackerChain),
				"winner_tip":     b64(ddWinnerTip),
				"added_signer_b": b64(ddB.Public),
			},
			"succession_weight": map[string]any{
				"genesis_hash":    b64(swGenesisHash),
				"honest_chain":    b64(swHonestChain),
				"attacker_chain":  b64(swAttackerChain),
				"winner_tip":      b64(swWinnerTip),
				"winner_signer_b": b64(swB.Public),
				"winner_signer_c": b64(swC.Public),
				"loser_signer_a":  b64(swA.Public),
			},
		},
	}
...
func TestRevokeWithReplacementGoldenVector(t *testing.T) {
	...
	sigA := decodeB64(t, v["winner_signer_a"])
	sigB := decodeB64(t, v["winner_signer_b"])
	sigD := decodeB64(t, v["winner_signer_d"])
	sigC := decodeB64(t, v["loser_signer_c"])
