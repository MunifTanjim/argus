...
// Genesis returns a copy of the currently recorded genesis, or nil if never
// tripped or observed. Trip keeps the first genesis it saw; Observe replaces it
// with the latest — so this reports the first after Trip, or the most recent after
// Observe. A tripped gate always reports non-nil (empty when Trip was handed no
// genesis), so callers can never infer "open" from a nil read.
