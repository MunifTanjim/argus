...
// answered: a protocol-level error reply still proves the remote is alive and
// processing frames — only a transport failure (closed connection, no reply within
// the timeout) means the link is gone. Treating an error reply as a miss would let
// a remote's dispatch policy tear down its own healthy link.
