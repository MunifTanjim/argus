...
const (
	...
	// gateway strictly less than a timer tick does — the pull is rate-limited. A
	// forged or withheld notification changes only when the pull happens, never what
	// the node accepts or concludes.
...
const (
	...
	NodeEventTrustChanged = "trust-changed" // gateway→client: trust log changed; client should pull promptly
)
