...
// structured task list (Claude Code's TaskCreate/TaskUpdate tools). Agents
// without one simply don't implement it, so change-detection and reads live in
// one place server-side rather than in each client.
type TaskSource interface {
	...
	// may have changed (the agent's own task-tool calls plus, for teams,
	// teammate messages that stand in for their untailed updates). The poller
	// re-reads the transcript each tick anyway, so this adds no I/O; the count
	// only grows, so a rise means new activity to push. hasTaskTool reports
	// whether any task-tool call is present — proof that tasks exist for this
	// session, letting the caller gate teammate-only activity without a disk hit.
