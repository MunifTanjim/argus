...
func TestE2EPlaintextGateway(t *testing.T) {
	agg := gateway.New(time.Second, false)
	reg := registry.New()
	src := gateway.NewInProcessSource("local-node", "Local Node", "itest", api.NodeCapabilities{}, reg, nil)
	agg.AddSource(src)
	...
	conn, err := api.DialWSConn(ctx, wsURL(ts.URL, "/client"), "", nil)
	if err != nil {
		t.Fatalf("dial: %v", err)
	}
