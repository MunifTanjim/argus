...
// TestLockInitReachesPeersWithoutWaitingForTheTick keeps the sync interval at its
// production length, so the only way node-b can learn the network was locked is the
// out-of-band offer lock.init makes. Before that offer existed, an operator who ran
// `lock init` saw peers report "locked mode: disabled" with no pin to adopt until
// the initiating node's next tick — five minutes later.
func TestLockInitReachesPeersWithoutWaitingForTheTick(t *testing.T) {
	...
	waitFor(t, "node-b sees the chain and quarantines", func() bool { return nodeB.Quarantined() })

	// The gateway holds the chain too, which is what makes `argus lock pin` on
	// node-b able to find a genesis to adopt.
