...
// requestRosterSync schedules a rate-limited roster refresh over the active
// uplink. Used when handleBeaconDeliver encounters an unknown beacon pub so
// newly joined peers become attributable within seconds rather than waiting a
// full roster-ticker interval. No-op when no uplink is connected.
