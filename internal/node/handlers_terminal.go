...
func (d *Node) handleTerminalOpen(ctx context.Context, params json.RawMessage) (any, error) {
	...
	if err != nil {
		// The adopted viewer pane may have died out-of-band; respawn it and retry
		// once so the user is not forced to refresh (recoverViewerPane no-ops for a
		// pane that is the session's own process, leaving the original error).
