...
// recordingTrustPeer is a trustCaller that records nodes.list calls and serves
// a configurable roster, used by beacon roster-refresh tests.
...
func (p *recordingTrustPeer) Call(method string, _, out any) error {
	p.mu.Lock()
	defer p.mu.Unlock()
	if method == api.MethodNodesList {
		p.rosters++
...
func TestUnknownBeaconPubRefreshesTheRoster(t *testing.T) {
	...
	waitFor(t, func() bool { return peer.rosterCount() >= 1 })
	waitFor(t, func() bool {
		d.peerBeaconMu.Lock()
...
func TestRepinningTheSameGenesisKeepsBeaconState(t *testing.T) {
	...
	res, err := beaconCallLockInit(t, d, api.LockInitParams{})
	if err != nil {
		t.Fatalf("init: %v", err)
	}
	d.peerBeaconMu.Lock()
...
// beaconNewLockTestNode creates a Node with a temp trust-chain path and a signer
// key, suitable for lock.init tests in the beacon test file.
func beaconNewLockTestNode(t *testing.T) *Node {
	t.Helper()
	d := New()
...
// beaconCallLockInit issues lock.init with the node's own signer automatically
// prepended when the caller did not list it.
func beaconCallLockInit(t *testing.T, d *Node, p api.LockInitParams) (api.LockInitResult, error) {
	t.Helper()
	own := d.SignerPublic()
	has := false
