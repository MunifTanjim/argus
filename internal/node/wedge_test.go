...
// wedgeWarn fires exactly once as the consecutive-decrypt-failure counter crosses
// the threshold, so a sustained wedge logs one warning rather than a per-frame flood.
...
// A reset (a successful decrypt) must require another full run before warning again.
