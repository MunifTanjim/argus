...
// TestLoadOrCreateBeaconKeyFailsLoudOnCorrupt verifies a present-but-corrupt
// beacon key file is NOT silently regenerated (which would change the node's
// beacon identity); LoadOrCreateBeaconKey errors and leaves the file untouched.
func TestLoadOrCreateBeaconKeyFailsLoudOnCorrupt(t *testing.T) {
	...
	for name, content := range cases {
		t.Run(name, func(t *testing.T) {
			...
			if _, err := LoadOrCreateBeaconKey(path); err == nil {
				t.Fatal("expected an error for a corrupt beacon key file, got nil")
			}
			...
			if !bytes.Equal(got, content) {
				t.Fatal("corrupt beacon key file must not be overwritten with a fresh key")
			}
		})
	}
}
