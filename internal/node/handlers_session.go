...
func (d *Node) handleTranscriptView(_ context.Context, params json.RawMessage) (any, error) {
	...
	return d.adapterFor(s.Tool).ReadTranscriptView(s.TranscriptPath)
}
...
func (d *Node) handleSessionToolDetail(_ context.Context, params json.RawMessage) (any, error) {
	...
	td, found, err := d.adapterFor(s.Tool).FindToolDetail(s.TranscriptPath, p.AgentID, p.ToolID)
	if err != nil {
		return nil, err
	}
	if !found {
...
func (d *Node) handleSessionInput(ctx context.Context, params json.RawMessage) (any, error) {
	...
	if p.Text != "" {
		if p.Prepare {
			if err := d.adapterFor(s.Tool).PrepareTextInput(ctx, c, s.Tmux.PaneID); err != nil {
				return nil, err
			}
		}
