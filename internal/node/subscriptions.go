...
func (d *Node) pollTranscript(ctx context.Context, n api.Notifier, subID, sessionID string, st adapter.StreamingTranscript, taskSignals func([]transcript.Chunk) (int, bool), sent []transcript.Chunk) {
	...
	for {
		...
		case <-t.C:
			...
			if taskSignals != nil {
				// Count only grows with task activity; resync the baseline on a
				// shrink (transcript rotation) but push only on a rise — and only
				// once a task tool call has appeared in the main transcript, which
				// proves tasks exist. Without one, a teammate message is just
				// chatter from a task-less team; the client still catches any
				// teammate-created tasks on open / manual refresh.
